                        ...
                        ?>
                                <div class="list-group-item list-group-item-action d-flex gap-3 py-3 bg-dark text-white mb-0" style="--bs-bg-opacity: .4;" aria-current="true">
                                    <div class="d-flex gap-2 w-100 justify-content-between">
                                        <div>
                                            <?php if ($modoEdicao): ?>
                                                <input type="text" class="form-control prato-item prato-nome" placeholder="Nome do Prato" name="pratos[<?php echo $pratoCount; ?>][nome]" value="<?php echo $prato['nome']; ?>" readonly>
                                                <input type="text" class="form-control prato-item prato-paragrafo" placeholder="Ingredientes" name="pratos[<?php echo $pratoCount; ?>][ingredientes]" value="<?php echo $prato['ingredientes']; ?>" readonly>
                                            <?php else: ?>
...
<script>
    ...
    // Script para adicionar pratos no modo de edição
    document.getElementById('add-prato').addEventListener('click', function() {
        const pratoCount = document.querySelectorAll('.prato-item').length / 3;
        const pratoContainer = document.createElement('div');
        pratoContainer.classList.add('list-group-item', 'list-group-item-action', 'd-flex', 'gap-3', 'py-3', 'bg-dark', 'text-white', 'mb-0');
        pratoContainer.style = '--bs-bg-opacity: .4;';
        pratoContainer.innerHTML = `
            <div class="d-flex gap-2 w-100 justify-content-between">
                <div>
                    <input type="text" class="form-control prato-item prato-nome" placeholder="Nome do Prato" name="pratos[${pratoCount}][nome]" value="" required>
                    <input type="text" class="form-control prato-item prato-paragrafo" placeholder="Ingredientes" name="pratos[${pratoCount}][ingredientes]" value="" required>
                </div>
                <div>
                    <div class="prato-preco-wrapper">
                        <input type="number" step="0.01" class="form-control prato-item prato-paragrafo prato-preco" placeholder="Preço" name="pratos[${pratoCount}][preco]" value="" required>
                    </div>
                    <button type="button" class="btn btn-danger mt-1" onclick="removerPrato(this)">Remover</button>
                </div>
            </div>
        `;
        document.getElementById('cardapio-container').appendChild(pratoContainer);
    });
